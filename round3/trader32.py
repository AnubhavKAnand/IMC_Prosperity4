...
- All trades happen at bid or ask (never inside). Posting one tick inside the
  bot quote captures flow (sellers prefer the highest bid; buyers prefer the lowest ask).
- Trade flow is mildly buy-skewed. Hard inventory caps (~50) are essential.
- Vol surface is tight around a parabolic smile IV(m) = a*m^2 + b*m + c, where m = log(K/S)/sqrt(T).
- Per-strike IV offsets applied (e.g. VEV_5400 structural underprice).
- VEV_6000/VEV_6500 pinned at bid=0/ask=1. Skipped.
- VEV_5000/5100/5200 see ~0 trades. Skipped for passive MM.
- VEV_4000/4500 track S - K. Light arb only.
- VEV_5300 has flow & spread; quote inside when spread permits.
"""
...
class Trader:
    def __init__(self):
        ...
        self.MM_PARAMS = {
            ...
            UNDERLYING: {"soft_pos": 20, "max_pos": 50, "passive_size": 20, "edge": 1, "take_edge": 3},
        }
    ...
    def run(self, state: TradingState):
        ...
        # Recover EWMA state across ticks
        try:
        ...
        # Persist dynamic smile state to next tick
        new_data = {
