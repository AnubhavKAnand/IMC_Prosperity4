...
- Trade flow is mildly buy-skewed. Hard inventory caps (~50) are essential or
  directional drift wipes us out.
- Vol surface is very tight: residual IV std ~0.003-0.007 around a parabolic
  smile IV(m) = 0.030*m^2 + 0.000*m + 0.240, where m = log(K/S)/sqrt(T).
- VEV_5400 has a *structural* IV underprice of -0.013 vs the smile, persistent
  across all 3 days. Excluded from smile fit, with a per-strike offset applied.
- VEV_6000/VEV_6500 pinned at bid=0/ask=1. Untradeable at +EV. Skipped.
- VEV_5000/5100/5200 see ~0 trades. Skipped.
- VEV_4000/4500 track S - K. Light arb only.
- VEV_5300/5400/5500 have flow; quote inside when spread permits.

Strategy:
1. HYDROGEL_PACK: market make at bid+1/ask-1 with size 20, hard cap pos=50,
   soft cap 20.
2. VELVETFRUIT_EXTRACT: same scheme.
3. VEV_4000/4500: take crossing trades against synthetic intrinsic S - K.
4. VEV_5300/5400/5500: quote inside the spread when spread >= 2, with strict
   inventory caps. Smile-fitted fair price (with VEV_5400 offset) acts as a
   safety filter.
"""
...
# Vouchers we run aggressive smile-arb on (cross the book when fair deviates).
ARB_VOUCHERS = ["VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500"]
# Vouchers we passively quote inside (only VEV_5300 has spread >= 2 reliably).
...
class Trader:
    def __init__(self):
        ...
        self.ARB_PARAMS = {
            "max_pos": 50,
            "threshold": 0.8,    # only cross when |bid - fair| or |ask - fair| > this
            "max_take_size": 30, # cap per-trade size
        }
        # Passive MM parameters for vouchers (only VEV_5300 is wide enough to sit inside)
