"""
Round 3 - Gloves Off: Live Calibrated Options Arbitrage & Delta Hedging

Refinements:
- Options volume dropped to 0 in previous runs because dynamic residual 
  tracking "over-fit" the model to the mid-price. 
- Restored STATIC structural IV_OFFSETS calibrated perfectly to the Day 3 sandbox.
- VEV_5400 is excluded from the dynamic smile fit so its massive structural 
  anomaly (-0.0223) doesn't drag down the rest of the volatility curve.
- Reverted arb threshold back to 0.8 to increase capture rate.
"""
...
# VEV_5400 is structurally broken. It is excluded to protect the curve.
SMILE_FIT_STRIKES = ["VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300", "VEV_5500"] 
ARB_VOUCHERS = ["VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500"]
PASSIVE_MM_VOUCHERS = ["VEV_5300"]
DEEP_ITM = ["VEV_4000", "VEV_4500"]

# Calibrated explicitly to the Day 3 Sandbox Environment
IV_OFFSETS = {
    "VEV_5000": 0.0021,
    "VEV_5100": -0.0030,
    "VEV_5200": -0.0028,
...
class Trader:
    def __init__(self):
        # Base coefficients fitted directly from Day 3 Sandbox data
        self.smile_a = -0.0505
        self.smile_b = 0.0161
        self.smile_c = 0.2701
        ...
        self.ARB_PARAMS = {
            "max_pos": 150,
            "threshold": 0.8, # Reverted back to 0.8 for tighter capture
            "max_take_size": 30, 
        }
    ...
    def run(self, state: TradingState):
        ...
        if S is not None and T > 0:
            for sym, pos in positions.items():
                if sym.startswith("VEV_"):
                    K = VOUCHER_STRIKES[sym]
                    m = math.log(K / S) / math.sqrt(T)
                    iv = self.smile_a * m * m + self.smile_b * m + self.smile_c + IV_OFFSETS.get(sym, 0.0)
                    iv = max(0.05, min(1.0, iv))
        ...
        if S is not None and T > 0:
            ...
            # Online smile fit (Excluding VEV_5400 structural anomaly)
